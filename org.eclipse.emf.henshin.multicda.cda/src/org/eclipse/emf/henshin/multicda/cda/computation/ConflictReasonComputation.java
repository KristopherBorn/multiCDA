...
public class ConflictReasonComputation {
	...
	private Set<ConflictReason> computeConflictReasons(ConflictReason current, Set<MinimalConflictReason> remaining) {
		...
		for (MinimalConflictReason mcr : remaining) {
			if (!haveCommonDeletionElement(current, mcr)) {
				ConflictReason initialReason = joinToNewInitialReason(current, mcr);
	 ...
	 *         possible
	 */
	private ConflictReason joinToNewInitialReason(ConflictReason span1, ConflictReason span2) {
		...
		ConflictReason newInitialReason = new ConflictReason(mappingsToR1, graph1Copy, mappingsToR2, originMCRs);
	...
	private Map<Node, Node> getS2toS1Map(ConflictReason span1, ConflictReason span2) {
