			...
			for (Graph graph : lhsAndRhsGraph) {
				for (Node node : graph.getNodes()) {
					for (Attribute attribute : node.getAttributes()) {
						if(attribute.getType().getClass().toString().endsWith("EAttributeImpl")){
							//most likely an enum in the metamodel
						}else if (!HenshinAGGExporter.isSupportedPrimitiveType(attribute.getType().getEType())) {
