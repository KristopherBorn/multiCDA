...
public class AttributedAtomCandidateComputation extends AtomCandidateComputation {
	...
	protected void addDeleteUseAtomCandidates(Rule rule1, Rule rule2, List<Span> result, ModelElement el1) {
		...
		if (el1 instanceof Node) {
			// nach sub- und super-typ �berpr�fen!
			// TODO: �berpr�fen, ob es Attribute mit abweichenden konstanten
			// Werten gibt!
			for (Node nodeInLhsOfR2 : rule2.getLhs().getNodes()) {
				...
				if (r1NodeIsSuperTypeOfR2Node || r2NodeIsSuperTypeOfR1Node || identicalType) {
					// F�r jedes Attribut der beiden Knoten muss gepr�ft
					// werden, ob es eine Konstante ist String und Zahlen.
					// Wenn es eine Konstante ist, dann muss �berpr�ft
					// werden, ob diese �bereinstimmen.
					// Wenn es eine Variable ist, so ist der potentielle
					// Konflikt nur vorhanden wenn die Variable f�r das
					...
					// TODO: hinzuf�gen und hier gebrauchen machen von einer
						...
						if (!differingConstants) {
							...
							if (attrOfSameTypeInLhsOfR1 != null) {
								...
								if (attrOfR2IsConstant && attrOfSameTypeInLhsOfR1IsConstant) {
									// pr�fen ob die Werte gleich sind.
								...
								if (attrOfR2IsConstant)
									...
									henshinFactory.createAttribute(newNodeInS1Graph, attrOfR2.getType(),
											attrOfR2.getValue());
								// TODO: wenn f�r das EAttribute nur eine
		...
		for (ModelElement el2 : atomicUsageElements) {
			...
			// Span S1span = new Span(rule1Mappings, S1, rule2Mappings);
			// result.add(S1span);
			// //TODO: newNodeInS1Graph m�ssen noch die jeweiligen Attribute
			// hinzugef�gt werden!
	...
	private void addChangeUseAtomCandidates(List<Span> result, Node el1) {
		...
		// TODO 
		// seperation of concern! Es handelt sich um zwei unterschiedliche F�lle
		// die zwar �hnlichkeiten aufweisen,
		// aber im Programmcode nicht miteinander vermischt werden sollten!
		// subroutinene die identisch sind k�nnen auch als gemeinsame Methode
		// existieren.
		// (Dazu die Sub-Methoden sauber definieren um m�gliche Unterschiede zu
		// identifizieren!)
		...
		for (Node nodeInLhsOfR2 : rule2.getLhs().getNodes()) {
			...
			// TODO: hier schon den m�glichen S-Graph mit dem passenden
			// Knoten erzeugen!
			...
			if (r1NodeIsSuperTypeOfR2Node || r2NodeIsSuperTypeOfR1Node || identicalType) {
				...
				// F�r jedes Attribut muss gepr�ft werden, ob es eine
				// Konstante ist String und Zahlen.
				// Genrell sind je Attribut folgende F�lle zu unterscheiden.
				// 1. Attr wird von R1 ge�ndert und von R2 benutzt
				// (ver�ndern kann create&delete des Attr sein, sowie
				// Wert�nderung �ber Konstanten und Variablen. Kurz gesagt:
				// LHS Value != RHS Value)
				...
				// auf)
				// 3. Attr ist in beiden Regeln und �ndert sich in R1 nicht.
				// Wenn es sich in lhsR1 und lhsR2 und Konstanten handelt
				// m�ssen diese �bereisntimmen.
				...
				for (Attribute attrOfR2 : nodeInLhsOfR2.getAttributes()) {
					if (!differingConstants) {
						...
						if (attrOfSameTypeInLhsOfR1 != null) {
							...
							// break early
							//
							// pr�fen ob bei R1 eine �nderung vorliegt
							...
							if (attrOfR2IsConstant && attrOfSameTypeInLhsOfR1IsConstant) {
								...
								// change?
								// wenn die erste Regel das Attr l�scht, so
								...
								// TODO: f�r VAR-VAR muss noch das Attribute
	...
	private boolean isAttrValueAConstant(Attribute attr, Rule rule) {
		 ...
		 * Anf�hrungszeichen haben und davon abgesehen parsable sein. - bei
		...
		// TODO: hier kann/sollte noch gepr�ft werden, ob es auch einen
	...
	private boolean isPrimitiveDataType(EAttribute type) {
		...
		if (eAttributeType != null) {
			...
			// TODO: muss noch vervollst�ndigt werden um (String, Char, Int,
	...
	private Set<Node> getAtomicChangeNodes() {
		...
		// change-use-conflicts) - (sammeln preserve Knoten mit ge�nderten
		// Attributen von R1)
		for (Node lhsNode : rule1.getActionNodes(preserveAction)) {
			...
			// an dieser Stelle wurde zwar identifiziert, dass es eine �nderung
			// des Attributwertes durch die
			// erste Regel in einem preserve Knoten gibt. Ob ein Knoten dieses
			// Typs aber �berhaupt in der zweiten
			// Regel genutzt wird ist hier noch nicht gekl�rt
		}
