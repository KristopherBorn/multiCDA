...
public class InitialReason extends Span {
	...
	private Set<ModelElement> getDeletionElementsOfSpan(Set<Mapping> mappingsOfSpanInRule1, Graph graph,
		...
		// alle Elemente im Graph des Span m�ssen gepr�ft werden, ob es sich
		// dabei um l�schende Elemente der ersten Regel handelt!
		// Kanten im Graph sind (f�r delete-use) immer l�schende Elemente (Das
	...
	public Set<ConflictReason> getAllDerivedConflictReasons(Set<ConflictAtom> uncoveredConflictAtoms) {
		...
		/*
		 * �ber jedes uncoveredCA iterieren jeweils f�r beide Knoten abarbeiten
		 * f�r jeden Knoten jeweils alle 'use'-Knoten der zweiten Regel finden!
		 * (auch Attr. ber�cksichtigen!) An jeder Position rekursiv versuchen
		 * auch weitere �berlappungskombinationen zu bilden! iteriere �ber
		 * Kombinationen f�r Knoten 1 - "Nullposition" ist ebenfalls g�ltig und
		 * behandelt den Fall, dass Knoten 2 nciht mit einbezogen ist iteriere
		 * �ber Positionen f�r Knoten 2 - "Nullposition" ist ebenfalls g�ltig
		 ...
		 * zu l�schenden Typs zwischen diesen gibt!
		 * 
		 */
		...
		// �ber jedes uncoveredCA iterieren
		for (ConflictAtom uncoveredCA : uncoveredConflictAtoms) {
			Set<Node> usedR1 = getUsedNodesOfR1();
			Set<Node> usedR2 = getUsedNodesOfR2();

			// TODO: bisher wird vererbung nicht ber�cksichtigt! K�nnen boundary
			// atoms auch bei Vererbung entstehen? Muss gepr�ft und ggf.
			// angepasst werden!
			...
			// jeweils f�r beide Knoten abarbeiten
			// f�r jeden Knoten jeweils alle 'use'-Knoten der zweiten Regel
			// finden! (auch Attr. ber�cksichtigen!)
			// An jeder Position rekursiv versuchen auch weitere
			// �berlappungskombinationen zu bilden!
			// iteriere �ber Kombinationen f�r Knoten 1 - "Nullposition" ist
			// ebenfalls g�ltig und behandelt den Fall, dass Knoten 2 nciht mit
			// einbezogen ist
			List<Node> potentialUsesN1R2 = new LinkedList<Node>(rule2.getLhs().getNodes(node1.getType()));
			potentialUsesN1R2.removeAll(usedR2);
			// Knoten aus R1 d�rfen nicht mehrfach in ein CR involviert sein!
	...
	private void processPotentialUsesN1R2(Set<ConflictAtom> uncoveredCAs, ConflictAtom uncoveredCA, Node node1,
		...
		// hier zuerst den Fall f�r node1 alleine (ohne node2) behandeln!
		...
		if (extendedCR != null) {
			...
			// iteriere �ber Positionen f�r Knoten 2 - "Nullposition" ist
			// ebenfalls g�ltig und behandelt den Fall, dass Knoten 1 nicht mit
			// einbezogen ist
			...
			for (Node potentialUseN2N2 : potentialUsesN2R2) {
				...
				if (!usedInR1 && !usedInR2) {
					...
					// keine Kante des zu l�schenden Typs zwischen diesen gibt!
					...
					// das hei�t, dass node1 und node2 genau so in Regel zwei
	...
	private boolean checkStoppingCriterion(ConflictAtom uncoveredCA, Node potentialUseInR2) {
		...
		// ohnehin hinf�llig.
		// TODO: f�r den zweiten Teil stimmt das doch nciht mehr, oder?!!
		if (this instanceof ConflictReason) {
			// Sonst muss gepr�ft werden,
			// 1. dass der use-Knoten noch nicht durch ein
			// additionalConflictAtom referenziert wird
			// (dieses w�rde sonst zum vollst�ndigen CA und zu Duplikaten in der
			// Ergebnissen f�hren)
			...
			// 2. dass f�r das neue uncovered CA der zweite Knoten noch nicht
			// vorhanden ist
			/*
