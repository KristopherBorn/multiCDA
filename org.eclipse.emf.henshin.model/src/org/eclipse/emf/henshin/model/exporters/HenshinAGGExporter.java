...
public class HenshinAGGExporter implements HenshinModelExporter {
	...
	// due to backward compatibilities due to added boolean parameter (KB 2017-08-21)
	@Override
	...
	public IStatus doExport(Module module, URI uri, boolean ignoreMultiplicities) {
		...
		try {
			...
			eclasses = advancedSortEClasses(eclasses); // TODO: (25.04.2017) die sortiermethode kann 'null' zurückgeben. Dann kommt es zu eienr NPE (z.B. bei UML Editierregeln)
			
//			eclasses = sortEClasses(eclasses);

			//TODO: vergleichen der Anzahl der sortierten Klassen mit beiden Sortiermethoden!!!!
			
			// sehr ähnlich zu nodeTypeIDs Map. --> ggf. vereinigen.
			Map<EClass, Element> mapOfCreatedElements = new HashMap<EClass, Element>();
			...
			for (EClass eclass : eclasses) {
				...
				mapOfCreatedElements.put(eclass, nodeTypeElem);
...
//							false);
//					String parentNodeTypeID = nodeTypeIDs.get(parentEClass); //kann 'null' zurück geben. z.B. bei vererbungszyklen. Was hat das zur Folge? Wie soll damit umgegangen werden?
//					if(parentNodeTypeID == null){
//						System.err.println("parentNodeTypeID = null !!!!");
//						System.err.println("parentNodeTypeID = null !!!!");
//						System.err.println("parentNodeTypeID = null !!!!");
//						System.err.println("parentNodeTypeID = null !!!!");
//					}
...
//				}
			...
			for (EClass eclass : eclasses) {
				Element nodeTypeElem = mapOfCreatedElements.get(eclass); 
				// Inheritance:
				for (EClass parentEClass : eclass.getESuperTypes()) {
					Element parentElem = newElement("Parent", nodeTypeElem,
							false);
...
//						System.err.println("parentNodeTypeID = null !!!!");
	...
	private static List<EClass> advancedSortEClasses(List<EClass> eclasses) {
		...
		while (mapOfUnsortedEClasses.size() > 0) {
			...
			EClass eClassWithLowestAmount = null;
			...
			while (iterator.hasNext() && processedEClass == null) {
				...
				if (entry.getValue() == 0) {
					...
					// festhalten der EClass mit der niedrigsten Anzahl an superclasses  
					if(entry.getValue() < lowestAmountOfSuperTypes){
						lowestAmountOfSuperTypes = entry.getValue();
						eClassWithLowestAmount = entry.getKey();
					}
				}
			}
			//TODO: wenn 'processedEClass' == null -> setzen der lowest EClass!
			if(processedEClass == null){
				processedEClass = eClassWithLowestAmount;
				sortedEClasses.add(processedEClass);
			}
			...
			// (25.04.2017): sollte nicht mehr auftreten nach Anpassung in der while-Schleife oben.
			if (processedEClass == null)
				return null;
		}
				...
				if (isSupportedPrimitiveType(attribute.getType().getEType())) {
					...
					if (constValue != null) {
						...
						if (t == ECORE.getEFloat()) { // float
							valueElem = newElement("Value", attrElem, false);
							Element floatElem = newElement("float",
									valueElem, false);
